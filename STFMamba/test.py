...
# Import the modified DATMamba model (which now uses CGDG+STFMamba cell)
...
parser.add_argument('--experiment_name', type=str, default='DATMamba', help='Unique experiment name, used to read results from directory')
...
# New argument for scene feature dimension used in CGDG
parser.add_argument('--scene_feature_dim', type=int, default=3, help='Dimension of global scene feature vector (e.g., density, avg speed, std speed)')
...
# Updated DATMamba initialization to include scene_feature_dim
model = DATMamba(
    input_feat_dim=4,
...
for i, graph_data in enumerate(test_dataset):
    ...
    # label: each graph_data has g.label
...
## Eval metrics for car detection
dict_eval = {}
...
print("Anomaly rate: ", sum(stretch_label_all) / len(stretch_label_all))

## Eval metrics for scene detection
dict_eval = {}
