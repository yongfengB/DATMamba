...
parser.add_argument('--padding', action='store_true', default=False, help='Whether input vehicle features are padded. Padded parts will be masked for loss calculation')
parser.add_argument('--data_edge_attr', type=str, default=None, help='Edge attribute type to be loaded. Not used for final model')
parser.add_argument('--train_file_name', type=str, default='simulation_processed', help='Parent directory for training data file')
    
# Model parameters
parser.add_argument('--edge_dropout', type=float, default=0., help='Edge dropout for graph layers')
...
parser.add_argument('--graph_layer', type=str, default='GATv2Conv', help='Graph layer to use: GATConv, GATv2Conv, TransformerConv, SAGEConv')
...
parser.add_argument('--n_head', type=int, default=3, help='# Attention heads')
parser.add_argument('--no_decode_graph', action='store_true', default=False, help='Whether to use graph information at decoding')
# New argument for scene feature dimension (used by CGDG)
parser.add_argument('--scene_feature_dim', type=int, default=3, help='Dimension of global scene feature vector (e.g., density, average speed, std speed)')

# Training parameters
parser.add_argument('--learning_rate', type=float, default=5e-2)
parser.add_argument('--lr_decay_step_size', type=int, default=50, help='Decrease learning rate by half every lr_decay_step_size epochs')
parser.add_argument('--clip_grad', type=float, default=1)
parser.add_argument('--loss_rate_lane', type=float, default=2, help='Loss weight for lane loss term')
parser.add_argument('--loss_rate_acc', type=float, default=2, help='Loss weight for acceleration loss term')
parser.add_argument('--loss_rate_speed', type=float, default=1, help='Loss weight for speed loss term')
parser.add_argument('--GPU', type=str, default="1")
parser.add_argument('--epochs', type=int, default=500)
parser.add_argument('--experiment_name', type=str, default='', help='Unique experiment name, used to save results to directory')
parser.add_argument('--verbal', action='store_true', default=False, help='Print training results')
parser.add_argument('--batch_size', type=int, default=64)
parser.add_argument('--highD_data', action='store_true', default=False, help='Train using HighD data. Otherwise use simulation data')
...
# Select dataset based on data type
if args.highD_data:
...
# Load training data list and partition into training and validation sets
...
# Initialize DATMamba model with the new scene_feature_dim parameter.
model = DATMamba(
    input_feat_dim=4,
    ...
    scene_feature_dim=args.scene_feature_dim
)
...
def train(epoch, train_loader, val_loader, best_val, best_epoch, verbal=False, padding=False):
    ...
    for graph_val in val_loader:
        ...
        if padding:
            ...
            graph_mask = torch.stack(graph_mask, dim=1)
        else:
            graph_mask = []
        loss = cal_loss(output, graph_x, rate_lane, rate_speed, rate_acc, graph_mask, n_lane)
    ...
    if verbal and epoch % 3 == 0:
        print('Train Epoch: {}  loss: {:e}  val_loss: {:e}'.format(epoch, loss_train, loss_val))
...
plt.plot(np.array(val_track), label='validation')
